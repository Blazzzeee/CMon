...
/* Allocation size */
#define ALLOC_SIZE (4ULL * 1024ULL * 1024ULL) // 4MB

/* Pages touched per allocation */
#define PAGE_TOUCHES 64
...
RunResult run_benchmark(int use_arena, int is_warmup, DecisionTable *table) {
    ...
    if (!is_warmup) {
        ...
        res.p50 = latencies[(size_t)(sample_count * 0.5)];
        res.p99 = latencies[(size_t)(sample_count * 0.99)];
...
void print_final_stats(const char *label, RunResult results[]) {
    ...
    printf("Speed:  %.3f M ops/sec\n", avg_speed / NUM_RUNS);

#if defined(__aarch64__)
    uint64_t freq = get_counter_freq();
    double tick_to_us = 1e6 / freq;

    printf("P50:    %.2f us\n", (avg_p50 / NUM_RUNS) * tick_to_us);
    printf("P99:    %.2f us\n", (avg_p99 / NUM_RUNS) * tick_to_us);
#else
...
int main() {
    ...
    for (int i = 0; i < 65536; i++) {
        ...
        table->should_zombie[i] = (rand() % 5 != 0); // 80% live
    }
